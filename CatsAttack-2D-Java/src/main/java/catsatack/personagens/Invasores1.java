 ...
 */
public class Invasores1 extends BasicElement {
    ...
    public Invasores1(int px, int py, Image image) {
        super(px, py);
        try {
    ...
    public void Update(long deltaTime) {
        ...
        if (jaColidiu()) {
            ...
            if (getX() >= getLMaxH() || getX() < getLMinH()) {
                ...
                setSpeed(Params.getInstance().nextInt(3) + 5);
    ...
    public void testaColisao(Character outro) {
        if (outro instanceof Invasores1 || outro instanceof Bomb) {
            return;
        } else {
